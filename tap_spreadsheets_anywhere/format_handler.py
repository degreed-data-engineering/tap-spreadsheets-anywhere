...
import tap_spreadsheets_anywhere.json_of_jsons_handler
...
def get_streamreader(
    uri, universal_newlines=True, newline="", open_mode="r", encoding="utf-8"
):
    kwarg_dispatch = {
        "azure": lambda: {
            "transport_params": {
...
def get_row_iterator(table_spec, uri):
    ...
    try:
        if format == "csv":
            ...
            reader = get_streamreader(
                uri, universal_newlines=universal_newlines, newline=None, open_mode="rb"
            )
